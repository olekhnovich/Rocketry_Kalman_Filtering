...
def initialize_covariance():
    ...
    P[3:6,3:6] *= 1.5**2     # velocity 
    P[6:9,6:9] = np.diag([(10*deg_to_rad)**2,
                          (10*deg_to_rad)**2,   
                          (np.pi)**2])     # attitude unknown
    P[9:12,9:12] *= 0.0001**2   # accel bias
    P[12:15,12:15] *= 0.0001**2 # gyro bias
    return P
...
gyro_ground_truth_csv = pd.read_csv('Defiance_Simulation_gyro_ground_truth.csv')
...
wx_deg = gyro_csv["wx"].values 
wy_deg = gyro_csv["wy"].values
wz_deg = gyro_csv["wz"].values

gyro_ground_truth_csv.columns = ["t", "wx", "wy", "wz"]
gyro_gt_t = gyro_ground_truth_csv["t"].values
gyro_gt_wx_deg = gyro_ground_truth_csv["wx"].values 
gyro_gt_wy_deg = gyro_ground_truth_csv["wy"].values
gyro_gt_wz_deg = gyro_ground_truth_csv["wz"].values
...
gyro_gt_wx_rad = np.radians(gyro_gt_wx_deg)
gyro_gt_wy_rad = np.radians(gyro_gt_wy_deg)
gyro_gt_wz_rad = np.radians(gyro_gt_wz_deg)
...
LEVER_ARM_BODY = np.array([0.0, 0.0, 0.563]) 
...
for i in range(len(ax)):
    ...
    gyro = np.array([gyro_gt_wx_rad[i], gyro_gt_wy_rad[i], gyro_gt_wz_rad[i]])
...
#wx_rad[0:10000] = gyro_gt_wx_rad[0:10000]
#wy_rad[0:10000] = gyro_gt_wy_rad[0:10000]
#wz_rad[0:10000] = gyro_gt_wz_rad[0:10000]
#at = (at+gt)/2  # Simple average to align time bases
...
def loop():
    ...
    state.quaternion = np.array(initial_quat_wlast)   # A guess. We likely won't know this. Account by setting initial uncertainty to be really high
    ...
    while imu_index < len(imu_data):
        ...
        if gps_index < len(gps_data):
            ...
            if abs(time_diff) < 1.0 and imu_index % 10 == 0:
...
fig5 = plt.figure(figsize=(14, 10)) # Increased size for readability
gs = gridspec.GridSpec(3, 2, figure=fig5)
fig5.suptitle('Position Estimation and Errors', fontsize=16)
# 2. Define Subplots
ax_left = fig5.add_subplot(gs[:, 0], projection='3d') 
ax_right_top = fig5.add_subplot(gs[0, 1])
ax_right_middle = fig5.add_subplot(gs[1, 1])
ax_right_bottom = fig5.add_subplot(gs[2, 1])

# 3. Plot Left (3D Position)
ax_left.plot(pos_log[:,0], pos_log[:,1], pos_log[:,2], label='Estimated Position')
ax_left.plot(pos_x, pos_y, pos_z, label='Ground Truth', linestyle='dashed')
ax_left.plot(pos_x_gps, pos_y_gps, pos_z_gps, label='GPS Position')
ax_left.legend()
ax_left.set_xlabel('X (m)')
ax_left.set_ylabel('Y (m)')
ax_left.set_zlabel('Z (m)')
ax_left.set_title('3D Position Plot') # Fixed: was 'ax.'

# 4. Plot Right Top (X Error)
ax_right_top.plot(t_log, pos_x_interp - pos_log[:,0], label='Position X Error')
ax_right_top.plot(t_log, pos_x_interp - pos_x_gps_interp, label='GPS X Error')
ax_right_top.plot(t_log, pos_x_interp - pos_x_interp, label='Ground Truth', color='black', alpha=0.3)
ax_right_top.set_title('Position X Error')
ax_right_top.set_ylabel('Error (m)')
ax_right_top.legend(fontsize='small')

# 5. Plot Right Middle (Y Error)
ax_right_middle.plot(t_log, pos_y_interp - pos_log[:,1], label='Position Y Error')
ax_right_middle.plot(t_log, pos_y_interp - pos_y_gps_interp, label='GPS Y Error')
ax_right_middle.plot(t_log, pos_y_interp - pos_y_interp, label='Ground Truth', color='black', alpha=0.3)
ax_right_middle.set_title('Position Y Error')
ax_right_middle.set_ylabel('Error (m)')
ax_right_middle.legend(fontsize='small')

# 6. Plot Right Bottom (Z Error)
ax_right_bottom.plot(t_log, pos_z_interp - pos_log[:,2], label='Position Z Error')
ax_right_bottom.plot(t_log, pos_z_interp - pos_z_gps_interp, label='GPS Z Error')
ax_right_bottom.plot(t_log, pos_z_interp - pos_z_interp, label='Ground Truth', color='black', alpha=0.3)
ax_right_bottom.set_title('Position Z Error')
ax_right_bottom.set_ylabel('Error (m)')
ax_right_bottom.set_xlabel('Time (s)')
ax_right_bottom.legend(fontsize='small')
